...
import { Beef, ChevronDown, Droplets, Egg, Fish, HelpCircle, Info, Milk, Sparkles, Utensils, Wheat, Wine } from 'lucide-react';
...
export default function Vasten() {
  ...
  const onthoudingenVoor = (niveau: (typeof dagVandaag)['vasten']['niveau']) => [
    { label: 'Vlees', toegestaan: ['vrij', 'geen'].includes(niveau), Icon: Beef },
    { label: 'Zuivel', toegestaan: ['vrij', 'geen', 'zuivel'].includes(niveau), Icon: Milk },
    { label: 'Eieren', toegestaan: ['vrij', 'geen', 'zuivel'].includes(niveau), Icon: Egg },
    { label: 'Vis', toegestaan: ['vrij', 'geen', 'zuivel', 'vis'].includes(niveau), Icon: Fish },
    { label: 'Olie', toegestaan: ['vrij', 'geen', 'zuivel', 'vis', 'wijn-olie', 'vastendag'].includes(niveau), Icon: Droplets },
    { label: 'Wijn', toegestaan: ['vrij', 'geen', 'zuivel', 'vis', 'wijn-olie', 'vastendag'].includes(niveau), Icon: Wine },
  ];
    ...
    <>
      ...
      <section className="orthodox-pattern parchment-pattern relative bg-parchment py-14 text-ink sm:py-20">
        <div className={CONTENT}>
          ...
          <div className="v17-fasting-today orthodox-pattern rounded-2xl border border-gold/40 bg-[#1c130d] px-6 py-8 text-cream shadow-[0_20px_45px_rgba(0,0,0,0.35)] sm:px-10 sm:py-10">
            ...
            <div className="fasting-food-grid mt-6 border-t border-gold/20 pt-5">
              {onthoudingenVandaag.map(({ label, toegestaan, Icon }) => (
          ...
          <div className="mt-8 grid gap-6 sm:grid-cols-2 lg:grid-cols-4">
            {(
              [
                { key: 'wat', title: 'Wat is vasten?', intro: 'Het typikon en de orde van de Kerk door het jaar heen.', Icon: HelpCircle },
                { key: 'hoe', title: 'Hoe vasten we?', intro: 'De ladder van het vasten, van vrij tot volledige onthouding.', Icon: Utensils },
                { key: 'periodes', title: 'Vastenperioden', intro: 'Vier grote vasten, verweven met feesten en uitzonderingen.', Icon: Wheat },
                { key: 'betekenis', title: 'De geestelijke betekenis', intro: 'Vasten als gebed, bekering en liefde tot de naaste.', Icon: Sparkles },
              ] as const
            ).map(({ key, title, intro, Icon }) => (
              <button
                key={key}
                type="button"
