      ...
      <div className="vandaag-frame">
        ...
        <div className="vandaag-paper">
          ...
          <div className="vandaag-list">
            <a className="vandaag-item" href="#adem"><CrossIcon/><div><b>Pijlgebed</b><em>“Heer Jezus Christus, ontferm U over ons.”</em></div><ChevronRight/></a>
            <a className="vandaag-item" href="#etmaal"><Church/><div><b>{uurMoment.naam}</b><em>“{uurMoment.tekst}”</em><small>{uurMoment.psalm} · Septuaginta</small></div><ChevronRight/></a>
            <a className="vandaag-item" href="#week"><CalendarDays/><div><b>Weekcyclus · {hoofdletter(dag.weekdagNaam)}</b><span>{weekthema.titel}</span></div><ChevronRight/></a>
            <a className="vandaag-item" href="#vasten"><Wheat/><div><b>Vasten vandaag</b><span>{dag.vasten.label}</span>{dag.vasten.periode && <small>{dag.vasten.periode}</small>}</div><ChevronRight/></a>
            <a className="vandaag-item" href="#pascha"><Flame/><div><b>Paschale cyclus</b><span>{dag.seizoen}{dag.toon ? ` · Toon ${dag.toon}` : ''}</span></div><ChevronRight/></a>
            <article className="vandaag-item vandaag-readings-item" role="link" tabIndex={0} onClick={(e)=>{ if ((e.target as HTMLElement).closest('button')) return; window.location.hash='kalender'; }} onKeyDown={(e)=>{ if(e.key==='Enter') window.location.hash='kalender'; }}><ScrollText/><div><b>Schriftlezingen</b>{lezingen.length ? lezingen.slice(0,2).map((l,i)=>{const refNl=vertaalRef(l.ref); const soort=lezingSoort(refNl); return <button key={`${l.ref}-${i}`} type="button" onClick={()=>openLezing({ref:l.ref,tag:l.tag,julianKey:dag.julianKey,civil:vandaag})}><span>{soort === 'evangelie' ? 'Evangelie' : soort === 'oud' ? 'Oude Testament' : 'Apostel'} · {refNl}</span></button>}) : <span>Leesrooster {LEZINGEN_JAAR}</span>}</div><a href="#kalender"><ChevronRight/></a></article>
            <a className="vandaag-item" href="#gebeden"><BookOpen/><div><b>Vaders &amp; moeders</b><span>Spreuk uit de woestijn</span><small>De verzameling wordt later toegevoegd.</small></div><ChevronRight className="vandaag-muted-chevron"/></a>
          </div>
